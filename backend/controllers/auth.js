...
exports.login = asyncWrapper(async (req, res = response, next) => {
...
exports.signup = asyncWrapper(async (req, res = response, next) => {
  const { email, password } = req.body;
  const emailExist = await User.findOne({ email });

  if (emailExist && emailExist.password) {
    return next(
      createCustomError('There is already one user with that email', 400)
    );
  }
...
exports.renewToken = asyncWrapper(async (req, res = response, next) => {
  const uid = req.uid;
